...
选择图片后不会自动打开裁剪界面或开始上传；图片会先进入待上传列表，方便多图拖拽排序。只有点击“裁剪并上传”按钮后，才会按当前列表顺序在浏览器本地打开裁剪界面，支持单图和多图逐张处理。每张图片都必须先进入裁剪流程；只有点击“跳过裁剪”时，才会直接使用原图进入原有压缩上传流程。每张图片都可以选择“确认裁剪并上传”“跳过裁剪”或“取消”。
...
5. 选择压缩预设和上传前重命名规则。
6. 拖拽或选择一张或多张图片；此时不会自动裁剪或上传，可以先按需要拖拽文件条目排序。
7. 确认顺序后点击“裁剪并上传”，然后程序会按当前顺序逐张打开裁剪弹窗；为每张图片选择比例并确认，或点击“跳过裁剪”。
